...
class PreActResNet_Encoder(nn.Module):
    def __init__(self, args, use_classifier, block, num_blocks, num_channels):
        ...
        for i in range(len(num_blocks)):
            ...
            # layers_array.append(nn.MaxPool2d(2, 2))
            if stride == 1:
                stride = 2
...
def PreActVGG16_Encoder(args, use_classifier):
    return PreActResNet_Encoder(args, use_classifier, PreActBlock, [1, 2, 3, 3, 3], [64, 128, 256, 512, 512])
...
def PreActResNetN_Encoder(args, use_classifier):
    if args.encoder == "vgg16":
        return PreActVGG16_Encoder(args, use_classifier)
