from cpc_models.VGG_Encoder import PreActResNetN_Encoder as vgg16
...
from baseline_models.VGG import PreActVGG
...
if __name__ == "__main__":
    ...
    if not args.fully_supervised:
        ...
        if args.encoder[:6] == "resnet":
            ...
            net = vgg16(args, use_classifier=True)
        ...
        if args.encoder[:6] == "resnet":
            ...
            net = PreActVGG(args).to(args.device)
        elif args.encoder[:10] == "wideresnet":
            parameters = args.encoder.split("-")
            depth = int(parameters[1])
            widen_factor = int(parameters[2])
